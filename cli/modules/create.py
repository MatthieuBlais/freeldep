...
@click.pass_context
def deployer(ctx, name, artifact_bucket, registry_table, wait, dryrun, output_location):
    ...
    registry_table = validate_registry(name, registry_table)

    click.echo(
        f"{name} Deployer: Creating a bucket {artifact_bucket} and registry table {registry_table}"
    )
    deployer = {"name": name, "artifact": artifact_bucket, "registry": registry_table}

    config = ctx.obj.config
    Compiler.build_template(
        deployer,
        InitializeDeployerTemplate.aws(deployer, config),
        wait=wait,
        dryrun=dryrun,
...
def subscription(ctx, name, deployer, emails, wait, dryrun, output_location):
    ...
    Compiler.build_template(
        deployer,
        SubscriptionDeployerTemplate.aws(name, emails, deployer, config),
        wait=wait,
        dryrun=dryrun,
        output_folder=output_location,
    )
...
def repository(ctx, deployer, wait, dryrun, output_location):
    ...
    Compiler.build_template(
        deployer,
        DeployerRepositoryTemplate.aws(deployer, config),
        wait=wait,
        dryrun=dryrun,
        output_folder=output_location,
    )
...
def project(ctx, name, deployer, branches, wait, dryrun, output_location):
    ...
    Compiler.build_template(
        deployer,
        ProjectTemplate.aws(name, branches, deployer, config),
        wait=wait,
        dryrun=dryrun,
        output_folder=output_location,
    )
